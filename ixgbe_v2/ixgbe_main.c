...
/* Symbiosis: export ixgbe_adapter globally */
...
int ixgbe_open(struct net_device *netdev)
{
	...
	printk(KERN_INFO "gadapter addr: %p\n", gadapter);
...
/* Symbiosis */
int ixgbe_sym_hello(unsigned int i)
{
  //struct ixgbe_ring *rx_ring;  
  struct ixgbe_q_vector *q_vector;
  ...
  printk(KERN_INFO "**** %s %u q_vector %s ***\n", __FUNCTION__, i, q_vector->name);
  printk(KERN_INFO "**** %s napi state=%lu weight=%d napi_id=%u ***\n", __FUNCTION__, napi->state, napi->weight, napi->napi_id);

  return 666;
}

void ixgbe_sym_napi_enable(void)
{
  printk(KERN_INFO " **** %s ****\n", __FUNCTION__);
...
void ixgbe_sym_napi_disable(void)
{
  struct ixgbe_hw *hw;
  hw = &gadapter->hw;
...
int ixgbe_sym_poll(unsigned int i, int budget)
{
  struct ixgbe_q_vector *q_vector;
  struct napi_struct *napi;
  int rc;
