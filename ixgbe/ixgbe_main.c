...
struct ixgbe_adapter *gadapter;
EXPORT_SYMBOL(gadapter);
...
int han_ixgbe_test(unsigned int i)
{  
  struct ixgbe_ring *rx_ring;  
  struct ixgbe_q_vector *q_vector;
  struct napi_struct *napi;
	
  q_vector = gadapter->q_vector[i];
  napi = &q_vector->napi;
  
  printk(KERN_INFO "**** han_ixgbe_test %u q_vector %s ***\n", i, q_vector->name);
  printk(KERN_INFO "**** han_ixgbe_test napi state=%lu weight=%d napi_id=%u ***\n", napi->state, napi->weight, napi->napi_id);
  ...
  ixgbe_for_each_ring(rx_ring, q_vector->rx) {
    ...
    printk(KERN_INFO "*** han_ixgbe_test rx_ring->next_to_clean = %u ***\n", rx_ring->next_to_clean);
...
int ixgbe_poll(struct napi_struct *napi, int budget)
{
	...
	pr_info("%s v_idx=%d budget=%d\n", __FUNCTION__, q_vector->v_idx, budget);
	...
	//printk(KERN_INFO "** about to run likely(napi_complete_done(napi, work_done)) work_done=%d q_vector->v_idx = %d budget=%d **\n", work_done, q_vector->v_idx, budget);
...
static int ixgbe_request_msix_irqs(struct ixgbe_adapter *adapter)
{
	...
	for (vector = 0; vector < adapter->num_q_vectors; vector++) {
		...
		printk(KERN_INFO "*** ixgbe_request_msix_irqs: %d %s ***\n", vector, q_vector->name);
...
void han_napi_enable(void)
{
  printk(KERN_INFO " **** han_napi_ENABLE ****\n");
  ixgbe_napi_enable_all(gadapter);
  ixgbe_irq_enable(gadapter, true, true);
}

void han_napi_disable(int work_done)
{
  struct ixgbe_hw *hw;
  ...
  printk(KERN_INFO " **** han_napi_DISABLE  ****\n");

  IXGBE_READ_REG(hw, IXGBE_EICR);
  ixgbe_irq_disable(gadapter);
  IXGBE_READ_REG(hw, IXGBE_EICR);
  
  ixgbe_napi_disable_all(gadapter);

  IXGBE_READ_REG(hw, IXGBE_EICR);
  ixgbe_irq_disable(gadapter);
  IXGBE_READ_REG(hw, IXGBE_EICR);
  //napi_disable(&gadapter->q_vector[i]->napi);  

  //for (q_idx = 0; q_idx < gadapter->num_q_vectors; q_idx++)
  //  printk(KERN_INFO " - napi_complete_done %d\n", napi_complete_done(&gadapter->q_vector[q_idx]->napi, work_done));
}

int han_ixgbe_poll(unsigned int i, int budget)
{
    struct ixgbe_q_vector *q_vector;
    struct napi_struct *napi;
    int rc;

    //printk(KERN_INFO " **** han_ixgbe_poll ****\n");
    
    q_vector = gadapter->q_vector[i];
    napi = &q_vector->napi;

    rc = ixgbe_poll(napi, budget);
    return rc;
}
